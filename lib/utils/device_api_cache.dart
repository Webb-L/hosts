...
class DeviceApiCache {
  ...
  /// 缓存hosts文件列表
  static Future<void> _cacheHostsFilesList(String ip, Directory cacheDir) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          if (responseData['success'] == true) {
            ...
            // 只保存data数组
            await hostsListFile.writeAsString(jsonEncode(hostsList));
  ...
  /// 缓存hosts文件内容
  static Future<void> _cacheHostsFileContent(String ip, String fileName, Directory cacheDir) async {
    try {
      final httpClient = HttpClient();
      httpClient.connectionTimeout = _scanTimeout;
      httpClient.idleTimeout = _scanTimeout;
  ...
  /// 缓存hosts文件历史
  static Future<void> _cacheHostsFileHistory(String ip, String fileName, Directory cacheDir) async {
    try {
      final httpClient = HttpClient();
      httpClient.connectionTimeout = _scanTimeout;
      httpClient.idleTimeout = _scanTimeout;
      ...
      if (response.statusCode == 200) {
        final responseBody = await response.transform(utf8.decoder).join();
        final historyDir = Directory(path.join(cacheDir.path, fileName, 'history'));
        ...
        try {
          ...
          if (responseData['success'] == true) {
            ...
            for (final historyItem in historyList) {
              ...
              if (historyId != null) {
                await _cacheHostsFileHistoryContent(ip, fileName, historyId, historyDir);
              }
            }
          }
        } catch (e) {
  ...
  /// 缓存hosts文件历史内容
  static Future<void> _cacheHostsFileHistoryContent(String ip, String fileName, String historyId, Directory historyDir) async {
    try {
      final httpClient = HttpClient();
      httpClient.connectionTimeout = _scanTimeout;
      httpClient.idleTimeout = _scanTimeout;
      
      final request = await httpClient.get(ip, _defaultPort, '/api/hosts/$fileName/history/$historyId');
      final response = await request.close();
  ...
  /// 获取缓存的设备API响应
  static Future<Map<String, dynamic>?> getCachedDeviceData(String ip) async {
    try {
      final cacheDir = await _getCacheDirectoryForDevice(ip);
      ...
      if (await hostsListFile.exists()) {
        final hostsListContent = await hostsListFile.readAsString();
        result['hostsList'] = hostsListContent;
      }
  ...
  /// 获取特定hosts文件的历史版本内容
  static Future<String?> getCachedHostsFileHistoryContent(String ip, String fileName, String historyId) async {
    try {
      final cacheDir = await _getCacheDirectoryForDevice(ip);
      final historyFile = File(path.join(cacheDir.path, fileName, 'history', historyId));
